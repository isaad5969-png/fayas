...
function StatCard({ emoji, label, value, color = 'text-purple-700' }) {
  return (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">⚙️ Administration</h1>
          <p className="text-gray-500 mt-1">Gérez les événements, utilisateurs et billets</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-8 flex-wrap">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-5 py-2.5 rounded-xl font-medium text-sm transition-all ${tab === t.key ? 'bg-purple-600 text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-purple-50 border border-gray-200'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'dashboard' && stats && (
        <div className="space-y-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard emoji="💰" label="Revenus totaux" value={`${stats.totalRevenue.toLocaleString()} MAD`} color="text-green-600" />
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {/* Recent tickets */}
            <div className="card p-6">
              <h3 className="font-bold text-gray-900 mb-4">Dernières réservations</h3>
              <div className="space-y-3">
                {stats.recentTickets.map(t => (
                  <div key={t.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div>
                      <p className="font-medium text-gray-800 text-sm">{t.user_name}</p>
                      <p className="text-xs text-gray-400 truncate">{t.event_title}</p>
                    </div>
                    <div className="text-right shrink-0 ml-4">
                      <p className="font-bold text-purple-700 text-sm">{t.total_price} MAD</p>
                      <p className="text-xs text-gray-400">{t.quantity} billet(s)</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Top events */}
            <div className="card p-6">
              <h3 className="font-bold text-gray-900 mb-4">Top événements</h3>
              <div className="space-y-3">
                {stats.topEvents.map((e, i) => (
                  <div key={e.id} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-gray-500 shrink-0">{e.tickets_sold}/{e.capacity}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === 'events' && (
        <div>
          ...
          {showForm && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold">{editId ? 'Modifier l\'événement' : 'Nouvel événement'}</h3>
                ...
                <form onSubmit={saveEvent} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select name="type" className="input bg-white" value={formData.type} onChange={handleField}>
                    ...
                    {formData.type === 'universite' && (
                      <div>
                        ...
                        <select name="university_id" className="input bg-white" value={formData.university_id} onChange={handleField}>
                    ...
                    {editId && (
                      <div>
                        ...
                        <select name="status" className="input bg-white" value={formData.status} onChange={handleField}>
          ...
          <div className="card overflow-x-auto">
              ...
              <tbody className="divide-y divide-gray-50">
                {events.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`badge ${e.type === 'gala' ? 'bg-purple-100 text-purple-700' : e.type === 'soiree' ? 'bg-indigo-100 text-indigo-700' : e.type === 'universite' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                        {e.type}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-semibold ${e.tickets_sold / e.capacity > 0.8 ? 'text-red-500' : 'text-gray-700'}`}>
                        {e.tickets_sold}/{e.capacity}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-medium text-purple-700">{e.price_standard} MAD</td>
                    <td className="px-4 py-3">
                      <span className={`badge ${e.status === 'published' ? 'bg-green-100 text-green-700' : e.status === 'draft' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                        {e.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => openEdit(e)} className="text-purple-600 hover:text-purple-700 font-medium text-xs px-2 py-1 rounded-lg hover:bg-purple-50">
                          ✏️ Modifier
                        </button>
                        <button onClick={() => deleteEvent(e.id)} className="text-red-500 hover:text-red-600 font-medium text-xs px-2 py-1 rounded-lg hover:bg-red-50">
                          🗑️ Supprimer
                        </button>
                      </div>
                    </td>
            ...
            {events.length === 0 && (
              <div className="text-center py-12 text-gray-400">Aucun événement. Créez le premier !</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'users' && (
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-6">{users.length} utilisateurs inscrits</h2>
          <div className="card overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
              ...
              <tbody className="divide-y divide-gray-50">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{u.university_name || '—'}</td>
                    <td className="px-4 py-3 font-medium text-purple-700">{u.ticket_count}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-400 text-xs">{new Date(u.created_at).toLocaleDateString('fr-MA')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'tickets' && (
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-6">{allTickets.length} billets vendus</h2>
          <div className="card overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
              ...
              <tbody className="divide-y divide-gray-50">
                {allTickets.map(t => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-800 max-w-xs truncate">{t.event_title}</p>
                      <p className="text-xs text-gray-400">{t.city} — {t.date}</p>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`badge ${t.ticket_type === 'vip' ? 'bg-amber-100 text-amber-700' : 'bg-purple-100 text-purple-700'}`}>
                        {t.ticket_type === 'vip' ? '⭐ VIP' : 'Standard'}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-medium text-gray-700">{t.quantity}</td>
                    <td className="px-4 py-3 font-bold text-purple-700">{t.total_price} MAD</td>
                    <td className="px-4 py-3 text-gray-400 text-xs">{new Date(t.purchased_at).toLocaleString('fr-MA')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {allTickets.length === 0 && (
              <div className="text-center py-12 text-gray-400">Aucun billet vendu pour le moment.</div>
            )}
          </div>
        </div>
      )}
