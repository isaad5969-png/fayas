...
const TIER_STYLES = {
  Bronze:  { card: 'from-amber-700 via-amber-800 to-amber-950', text: 'text-amber-600', bg: 'bg-amber-50',  ring: 'ring-amber-400', bar: 'bg-amber-500' },
  Silver:  { card: 'from-slate-400 via-slate-500 to-slate-700', text: 'text-slate-500', bg: 'bg-slate-50',  ring: 'ring-slate-400', bar: 'bg-slate-400' },
  Gold:    { card: 'from-yellow-400 via-amber-500 to-orange-600',text: 'text-amber-500', bg: 'bg-yellow-50', ring: 'ring-yellow-400', bar: 'bg-yellow-400' },
  Platine: { card: 'from-violet-600 via-purple-700 to-indigo-900',text: 'text-violet-600', bg: 'bg-violet-50', ring: 'ring-violet-400', bar: 'bg-violet-500' },
}
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-100">
        <div className="text-center p-10 bg-white rounded-3xl shadow-xl max-w-md border border-purple-100">
          <div className="w-16 h-16 bg-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg shadow-purple-200">
            <Icon name="gem" className="w-8 h-8 text-white" />
          </div>
          <h2 className="text-2xl font-extrabold text-gray-900 mb-3">Programme BilletCoins</h2>
          <p className="text-gray-500 mb-6">Connectez-vous pour accéder à votre espace fidélité et consulter vos avantages exclusifs.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-purple-50/30 to-indigo-50/40">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-10">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-purple-100 text-purple-700 px-4 py-2 rounded-full text-sm font-bold mb-4">
              ...
              Programme BilletCoins
            </div>
            <h1 className="text-4xl font-extrabold text-gray-900">Votre carte de fidélité</h1>
            <p className="text-gray-500 mt-2">Chaque dirham dépensé vous rapproche du Platine</p>
          </div>
            ...
            <div className={`relative w-full max-w-sm h-52 rounded-3xl bg-gradient-to-br ${style.card}
              ...
              <div className="relative h-full flex flex-col justify-between p-6">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-white/60 text-xs font-medium tracking-widest uppercase">BilletterieMa</p>
                    <p className="text-white font-extrabold text-lg leading-tight">BilletCoins</p>
                  </div>
          ...
          {nextTier && (
            <div className="mt-6 max-w-sm mx-auto">
              ...
              <p className="text-center text-sm text-gray-500 mt-2">
                Encore <span className="font-bold text-purple-700">{(nextTier.min - points).toLocaleString()} pts</span> pour atteindre {nextTier.emoji} <strong>{nextTier.name}</strong>
              </p>
            </div>
          )}
          {!nextTier && (
            <p className="text-center text-purple-700 font-bold mt-4 flex items-center justify-center gap-2">
        ...
        {/* ════════ Onglets ════════ */}
        <div className="flex gap-1.5 mb-8 bg-white rounded-2xl p-1.5 shadow-sm border border-gray-100 overflow-x-auto">
          {[
        ...
        {activeTab === 'apercu' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { icon: '💰', value: `${points.toLocaleString()}`, label: 'BilletCoins',    color: 'text-purple-700' },
                { icon: '🎫', value: stats.totalTickets,           label: 'Billets achetés', color: 'text-blue-700'   },
                { icon: '🏙️', value: stats.citiesVisited.length,   label: 'Villes visitées', color: 'text-emerald-700'},
                { icon: '💳', value: `${stats.totalSpent} MAD`,    label: 'Total dépensé',   color: 'text-amber-700'  },
              ].map(s => (
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 className="font-extrabold text-gray-900 text-lg mb-4 flex items-center gap-2">
                <Icon name="bolt" className="w-5 h-5 text-purple-600" />
                Comment gagner des BilletCoins
              </h3>
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  { icon: '🎫', title: 'Achat standard',   desc: '1 MAD = 1 BilletCoin',           badge: '×1'   },
                  { icon: '⭐', title: 'Achat VIP',         desc: '1 MAD VIP = 1.5 BilletCoins',    badge: '×1.5' },
                  ...
                  <div key={m.title} className="flex items-center gap-3 p-3 bg-purple-50 rounded-xl">
                    <span className="text-2xl">{m.icon}</span>
                    <div className="flex-1">
                      <p className="font-semibold text-gray-800 text-sm">{m.title}</p>
                      <p className="text-xs text-gray-500">{m.desc}</p>
                    </div>
                    <span className="font-extrabold text-purple-700 text-sm bg-purple-100 px-2 py-1 rounded-lg">{m.badge}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {badges.map(badge => (
                <div key={badge.id}
                  ...
                  <p className={`font-extrabold text-sm ${badge.earned ? 'text-purple-800' : 'text-gray-400'}`}>{badge.label}</p>
              ...
              return (
                <div key={t.name}
                  className={`rounded-2xl overflow-hidden border-2 transition-all ${
                    isCurrent ? `border-purple-500 shadow-lg shadow-purple-100` : 'border-gray-100'
                  }`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h3 className="font-extrabold text-gray-900 flex items-center gap-2">
                <Icon name="history" className="w-4 h-4 text-purple-600" />
                Historique des BilletCoins gagnés
              </h3>
            </div>
            {history.length === 0 ? (
              <div className="p-10 text-center text-gray-400">
                <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-3">
                  <Icon name="ticket" className="w-6 h-6 text-gray-400" />
                </div>
                <p className="font-semibold text-gray-600">Aucun achat pour l'instant</p>
                ...
                {history.map(h => (
                  <div key={h.id} className="px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition-colors">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      h.ticket_type === 'vip' ? 'bg-amber-100 text-amber-600' : 'bg-purple-100 text-purple-600'
                    }`}>
                      <Icon name={h.ticket_type === 'vip' ? 'star' : 'ticket'} className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-800 text-sm truncate">{h.event_title}</p>
                      <p className="text-xs text-gray-400">{h.city} · {formatDate(h.purchased_at)} · {h.quantity} billet(s) {h.ticket_type.toUpperCase()}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-extrabold text-purple-700">+{h.points_earned.toLocaleString()}</p>
                      <p className="text-xs text-gray-400">BilletCoins</p>
                    </div>
                  </div>
                ))}
              </div>
